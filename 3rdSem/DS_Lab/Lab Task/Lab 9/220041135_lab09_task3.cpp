...
void display(TrieNode* root, char str[], int level, string prefix)
{
    ...
    for (int i = 0; i < 26; i++)
    {
        ...
        {
            str[level] = i + 'a';
            display(root->childNode[i], str, level + 1, prefix);
        }
    }
}
...
bool search_provider(TrieNode * currentNode, string t)
{
    char str[100];
    for (auto c : t)
    {
...
//    display(root, str, 0);
    ...
    {
        t+=c;
        bool checker = search_provider(root, t);
        if(!checker)
            cout << "Null";
        cc = 0;
        cout << endl;
